...
export const CampusMap: React.FC<CampusMapProps> = ({ locations }) => {
  ...
  const getLocationColor = (type: string) => {
    switch (type) {
      ...
      case 'facility': return 'bg-green-600';
      case 'parking': return 'bg-purple-600';
      case 'dining': return 'bg-orange-600';
          ...
          {selectedLocation ? (
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
              <div className="flex items-start space-x-3 mb-4">
                <div className={`w-10 h-10 ${getLocationColor(selectedLocation.type)} rounded-lg flex items-center justify-center text-white text-lg`}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div className="space-y-2">
              {locations.slice(0, 4).map((location) => (
                ...
                >
                  <div className={`w-6 h-6 ${getLocationColor(location.type)} rounded flex items-center justify-center text-white text-xs`}>
                    {getLocationIcon(location.type)}
                  </div>
