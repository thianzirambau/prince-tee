...
import { Announcement, Booking, User } from '../types';
...
export const Dashboard: React.FC<DashboardProps> = ({ user, announcements, bookings, onTabChange }) => {
  const quickStats = [
    ...
    { label: 'Credits Completed', value: `${user.creditsCompleted}/${user.totalCredits}`, icon: BookOpen, color: 'text-blue-500' },
    { label: 'Active Bookings', value: bookings.filter(b => b.status === 'confirmed').length.toString(), icon: Calendar, color: 'text-blue-700' },
    { label: 'Unread Alerts', value: announcements.filter(a => !a.isRead).length.toString(), icon: Bell, color: 'text-blue-800' },
  ];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-blue-600 to-blue-700 rounded-2xl p-8 text-white">
        ...
        <p className="text-blue-100 text-lg">Ready to make the most of your campus experience today?</p>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="p-6 space-y-4">
            {recentAnnouncements.map((announcement) => (
              <div key={announcement.id} className="flex items-start space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors cursor-pointer">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100">
          ...
          <div className="p-6 space-y-4">
            {upcomingBookings.map((booking) => (
              <div key={booking.id} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-50 transition-colors">
